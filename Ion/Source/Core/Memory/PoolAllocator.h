...
inline void TestPoolAllocator()
{
	...
	for (int32 i = 0; i < 10; ++i)
	{
		LOG_TRACE("{0} - {1:5} - {2}", allocs[i]->Text, BoolStr(GET_POOL_META_ALLOC_FLAG(*(uint64*)(allocs[i] + 1))), (void*)allocs[i]);
	}
	pool.Free(allocs[0]);
	pool.Free(allocs[2]);
	pool.Free(allocs[5]);
	...
	for (int32 j = 0; j < 10; ++j)
	{
		...
		timer.Stop();
		(void)test;
		LOG_INFO("{0} allocations - {1:.6f}ms", nAllocs, timer.GetTime(EDebugTimerTimeUnit::Millisecond));
	}
	LOG_INFO("Malloc:");
	for (int32 j = 0; j < 10; ++j)
	{
