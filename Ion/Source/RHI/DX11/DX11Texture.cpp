...
namespace Ion
{
	...
	Result<void, RHIError> DX11Texture::CopyTo(const std::shared_ptr<RHITexture>& destination) const
	{
		ID3D11DeviceContext* context = DX11::GetContext();

		std::shared_ptr<DX11Texture> destTexture = std::static_pointer_cast<DX11Texture>(destination);
	...
	Result<void, RHIError> DX11Texture::CreateTexture(const TextureDescription& desc)
	{
		...
		DXGI_FORMAT resourceFormat = DXCommon::TextureFormatToDXGIFormat(desc.Format, EDX11FormatUsage::Resource);
	...
	Result<void, RHIError> DX11Texture::CreateViews(const TextureDescription& desc)
	{
		...
		if (desc.bUseAsRenderTarget)
		{
			...
			rtvDesc.Format = DXCommon::TextureFormatToDXGIFormat(desc.Format, EDX11FormatUsage::RTV);
		...
		if (desc.bUseAsDepthStencil)
		{
			...
			dsvDesc.Format = DXCommon::TextureFormatToDXGIFormat(desc.Format, EDX11FormatUsage::DSV);
	...
	Result<void, RHIError> DX11Texture::CreateSampler(const TextureDescription& desc)
	{
		...
		if (desc.bCreateSampler)
		{
			...
			srvDesc.Format = DXCommon::TextureFormatToDXGIFormat(desc.Format, EDX11FormatUsage::SRV);
